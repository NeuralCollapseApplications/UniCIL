...
from typing import Mapping, Sequence, Union, Optional, Tuple, List, Dict, Callable
...
ORDER_2023 = [
    13,188,96,122,85,2,61,173,38,8,50,4,16,190,158,21,169,156,118,165,
    98,147,32,23,123,82,92,155,137,91,141,9,185,17,196,59,170,113,163,168,
    68,80,136,140,76,64,33,120,172,143,157,81,182,30,43,150,151,41,99,148,
    109,126,130,11,84,35,45,73,48,178,153,14,10,195,18,149,107,70,57,192,
    12,142,194,20,75,105,134,5,62,154,102,106,42,74,27,44,1,128,97,103,
    186,95,0,117,171,179,56,47,94,90,132,180,40,164,71,146,67,79,89,193,
    24,100,54,144,86,26,60,124,167,184,66,93,36,189,37,78,46,197,119,6,
    125,55,83,7,138,133,121,31,152,19,51,108,69,129,139,104,162,199,159,115,
    187,174,34,198,135,191,65,72,88,127,111,15,166,53,177,112,114,49,29,160,
    176,63,110,28,58,131,39,77,175,181,145,22,101,52,116,3,161,183,25,87,
]


@DATASETS.register_module()
class CUBCIL200(Dataset):
    ...
    def __init__(
        ...
        if is_lt:
            ...
            img_num_per_cls = self.get_img_num_per_cls(cls_num=self.num_cls, imb_factor=lt_factor)
            if lt_shuffle:
            ...
            self.log_class_num()
            assert not test_mode
            img_num_per_cls = [fs_num] * self.num_cls
            self.gen_imbalanced_data(img_num_per_cls)
            self.log_class_num()
        else:
            self.log_class_num()
    ...
    def get_fs_data(self):
        ...
        for data_info in data_infos:
            label = data_info['cls_id']
            if data_info['img_info']['filename'].split('/')[-1] not in FSCIL_SAMPLES[CLASSES[label]]:
                continue
            else:
                cls_cnt[label] += 1
                self.data_infos.append(data_info)
    ...
    def load_annotations(self):
        ...
        id2image = list2dict(mmcv.list_from_file(image_file))
        id2train = list2dict(mmcv.list_from_file(split_file))
        id2class = list2dict(mmcv.list_from_file(class_file))
        train_idx = []
        test_idx = []
        ...
        if self.subset == 'train':
            for k in train_idx:
                ...
                if CLASSES[cls_id] in self.CLASSES:
                    info = {
                        'img_prefix': os.path.join(self.data_prefix, 'images'),
