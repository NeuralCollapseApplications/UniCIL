_base_ = [
    '../_base_/models/resnet_etf_dist_aug.py',
    '../_base_/datasets/cifar100.py',
    '../_base_/schedules/cifar100_200e.py',
    '../_base_/default_runtime.py'
]

total_cls_num = 100
base_cls_num = 50
cls_num_step = 2

checkpoint_config = dict(interval=50, max_keep_ckpts=2)
evaluation = dict(interval=50, start=0)

model = dict(
    backbone=dict(
        _delete_=True,
        type='ResNet18',
        norm_cfg=dict(type='SyncBN')
    ),
    neck=dict(type='MLPBNNeck', in_channels=512, out_channels=2048, norm_cfg=dict(type='SyncBN')),
    head=dict(
        type='ETFHeadInc',
        in_channels=2048,
        num_classes=1000,
        loss=dict(type='DRLoss', loss_weight=10.),
        with_len=False,
    ),
    dist_mode='pre_logits_old',
    loss_dist=dict(type='DRLoss', loss_weight=50.),
)

incremental_settings = dict(lr=5., use_lambda_lucir=True, vec_init=True)
incremental_update = dict(
    optimizer=dict(
        type='SGD',
        lr=0.25,
        momentum=0.9,
        weight_decay=0.0005,
        paramwise_cfg=dict(
            custom_keys={
                'backbone': dict(lr_mult=0.005, decay_mult=0.005),
            },
            norm_decay_mult=1.
        )
    ),
    optimizer_config=dict(_delete_=True, grad_clip=dict(max_norm=3.0)),
    log_config=dict(
        interval=4,
    )
)

use_general = True
general_settings = dict(
    seed = 2010,
    lt_factor=0.01,
    fs_num = 5,
)

work_dir = './work_dir/cifar_random_2010/cifar_r18_25t'

